...
const PROSE = [
  ...
  "More of the close is automatable than most teams assume. Connector pulls from REST APIs and SFTP run on their own schedule — hourly, nightly or weekly — so trial balance lands without anyone exporting it by hand, and Excel and CSV uploads land in the same cube. The SAP and NetSuite engines are built and tested; we set those up with you today rather than from a self-serve screen. Consolidation runs as computation — intercompany matching, non-controlling interests, dated ownership, period-average FX translation under IAS 21 — with a consolidation bridge and board pack as outputs. Variance movers are ranked by financial impact rather than eyeballed in a pivot. Reports export to CSV, Excel, or PDF from any view. Forecasting compares 15 algorithms on holdout windows using R², RMSE, MAE, and MAPE, and lets you pin the winner.",
...
const SECTION_1 = [
  {
    ...
    body:
      "Pulls from REST APIs and SFTP run on their own schedule — hourly, nightly or weekly — or start with a CSV/Excel upload. Trial balance lands in the cube without anyone exporting it by hand. The SAP and NetSuite engines are built and tested; we set those up with you today.",
  },
  {
    Icon: GitBranch,
    meta: "CSV · Excel · PDF",
